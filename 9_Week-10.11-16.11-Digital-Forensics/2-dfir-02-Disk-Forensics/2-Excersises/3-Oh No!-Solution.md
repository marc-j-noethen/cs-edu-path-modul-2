...
**Course:** Cyber Security Analyst - DFIR Forensics | **Date:** 11 November 2025
...
**Goal:**  
Simulate a full forensic workflow: create a virtual disk, accidentally delete a critical piece of information, "bag" the evidence by creating an image, and then analyze the image to recover the lost data.

**Requirements:**

- **Create the "Workstation Disk":**
    
- Open a terminal on your Kali VM. First, create a 50MB file to act as our virtual disk.
    
- Now, associate this file with a loopback device, making it appear like a real disk to the system.
    
- Format your new "disk" with an ext4 file system.
...
objective = 'Simulate a full forensic workflow: create a virtual disk, accidentally delete a critical piece of information, "bag" the evidence by creating an image, and then analyze the image to recover the lost data.'
evidence = 'The full command you used to find the lost password within the `my-disk.dd` image.'
...
```text
I answered the required sub-tasks for "Oh No!" and submitted the requested format.
```
