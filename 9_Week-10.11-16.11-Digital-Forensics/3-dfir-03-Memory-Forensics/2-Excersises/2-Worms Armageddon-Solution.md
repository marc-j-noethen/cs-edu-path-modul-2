...
**Course:** Cyber Security Analyst - Memory Forensics | **Date:** 12 November 2025
...
**Goal:**  
Analyze a memory dump infected with the Cridex malware, a notorious banking worm known for process injection and data theft.

**Requirements:**

- Download the Cridex memory dump:
    
- **Initial Triage:** Use  `windows.info.Info` to identify the correct profile or OS version.
    
- **Find the Intruder:** The Cridex malware injects itself into `explorer.exe` and then launches a suspicious child process. Use `pslist` and `pstree` to find a suspicious process whose parent is `explorer.exe` (PID 1484).
    
- **Find the C2:** Cridex communicates with command-and-control servers. Since this is a Windows XP dump, use the `connscan` plugin to find active network connections. Identify the foreign IP addresses connecting to the infected machine over port 8080.
...
objective = 'Analyze a memory dump infected with the Cridex malware, a notorious banking worm known for process injection and data theft.'
evidence = 'What is the OS version?'
...
```text
I answered the required sub-tasks for "Worms Armageddon" and submitted the requested format.
```
