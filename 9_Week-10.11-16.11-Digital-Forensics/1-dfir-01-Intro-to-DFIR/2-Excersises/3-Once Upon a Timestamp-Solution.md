...
**Course:** Cyber Security Analyst - DFIR | **Date:** 10 November 2025
...
**Goal:**  
Trace the lifecycle of a file from an external source to its use on a system by correlating USB connection artifacts, LNK file evidence, and program execution data.

**Requirements:**

- **Simulate USB:** Attach a virtual USB drive to your Windows VM. (In your VM software, create and attach a new small virtual disk as a USB device. Lookup online how to achieve that).
    
- **Create Evidence:** On the virtual USB drive, create a text file named `Project_Phoenix_Data.txt`. Copy this file from the USB drive to your VM's `Documents` folder. Then, open the copied file using `notepad.exe`.
    
- **Investigate:** Now, find the evidence of your actions.
    
- Find the LNK file created when you opened the document from your `Documents` folder (look in `%APPDATA%\\Microsoft\\Windows\\Recent` via cmd with `dir`). Analyze it with `LECmd.exe` to see the file path it points to.
...
```text
# Inputs
context = 'STIG / policy reference'
objective = 'Trace the lifecycle of a file from an external source to its use on a system by correlating USB connection artifacts, LNK file evidence, and program execution data.'
evidence = 'Submit a single screenshot that is composed of three smaller screenshots, showing:'
...
```text
I answered the required sub-tasks for "Once Upon a Timestamp" and submitted the requested format.
```
