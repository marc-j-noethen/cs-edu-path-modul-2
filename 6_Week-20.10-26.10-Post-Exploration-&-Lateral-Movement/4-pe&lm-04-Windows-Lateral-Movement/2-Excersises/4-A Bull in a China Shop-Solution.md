...
**Goal:**  
Use `psexec.py` to gain a remote shell, and then find both the file system and event log artifacts that this "loud" tool leaves behind.

**Requirements:**

- **Attack (Red Team):**
    
- From your Kali VM, use `psexec.py` to get a command shell on your Windows VM using the Administrator's password.
    
- **Analysis (Blue Team):**
    
- While the `psexec.py` session is still active, switch to your Windows VM.
...
objective = 'Use `psexec.py` to gain a remote shell, and then find both the file system and event log artifacts that this "loud" tool leaves behind.'
evidence = 'Submit a single screenshot of your Windows VM desktop. The screenshot must show two windows side-by-side:'
...
```text
I answered the required sub-tasks for "A Bull in a China Shop" and submitted the requested format.
```
