...
**Goal:**  
Understand the fundamentals of interacting with a Windows file share (SMB) and then find the corresponding access logs on the target machine.

**Requirements:**

- **Attack (Red Team):**
    
- From your Kali VM, use `smbclient` to list the available shares on your Windows VM using the `Administrator` username and password.
    
- Connect directly to the `Share` folder using `smbclient`.
    
- At the `smb: \\>` prompt, use `ls` to see the files and `get secret_plan.txt` to download the file.
...
```text
# Inputs
platform = 'NIST reference material'
objective = 'Understand the fundamentals of interacting with a Windows file share (SMB) and then find the corresponding access logs on the target machine.'
evidence = 'Submit a two screenshots:'
...
```text
I completed the referenced lab for "Share and Share Alike" and captured the requested evidence.
```
