...
**Goal:**  
Use online tools for WHOIS and passive DNS enumeration on `uber.com`, focusing on identifying subdomains to understand its public network structure.

**Requirements:**

- **WHOIS Intel:** For `uber.com`, use an online WHOIS tool to find its Registrar, Registration Date, and Name Servers. What does the Registrar choice suggest about their domain management?
    
- **Core DNS Records:** For `uber.com`, use an online DNS tool to find an A record and an MX record. What does the MX record imply about their email infrastructure?
    
- **Subdomain Quest:** Using passive tools, identify at least five distinct subdomains of `uber.com`. For each, list its full name and speculate on its likely purpose.
    
- **Subdomain Significance:** Why is discovering a company's subdomains valuable for security assessments?
...
objective = 'Use online tools for WHOIS and passive DNS enumeration on `uber.com`, focusing on identifying subdomains to understand its public network structure.'
evidence = 'Submit a PDF document containing:'
...
```text
I answered the required sub-tasks for "Officially-Registered" and submitted the requested format.
```
