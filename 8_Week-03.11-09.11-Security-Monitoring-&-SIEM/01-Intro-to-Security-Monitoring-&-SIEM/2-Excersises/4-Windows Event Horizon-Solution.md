# 🐍 Windows Event Horizon (01-Intro-to-Security-Monitoring-&-SIEM)
...
**Goal:**  
Generate specific security events on a Windows Virtual Machine, ingest the relevant Windows Event Log into your local Splunk instance, and perform an initial scan for user account changes. This exercise requires your local Splunk setup from the pre-class preparation.

**Requirements:**

- Ensure your Windows VM is running and you have PowerShell open as Administrator.
    
- Enable policy that monitors account creation or delete: `auditpol /set /subcategory:"User Account Management" /success:enable /failure:enable`
    
- **Generate Events:** Create and then delete a new local user account. For example:
    
- `New-LocalUser -Name "TempUser1" -NoPassword`
...
objective = 'Generate specific security events on a Windows Virtual Machine, ingest the relevant Windows Event Log into your local Splunk instance, and perform an initial scan for user account changes. This exercise requires your local Splunk setup from the pre-class preparation.'
evidence = 'A screenshot of your Splunk search results showing both Event ID 4720 and 4726 from your ingested file.'
...
```text
I answered the required sub-tasks for "Windows Event Horizon" and submitted the requested format.
```
