...
**Goal:**  
Use the OpenSSL command-line tool to inspect the details of a website's digital certificate and answer specific questions about its contents.

**Requirements:**

- Using the `openssl` command-line tool, connect to `google.com` and save its certificate to a file named `google.crt`.
    
- Once you have the certificate file, use `openssl x509` to parse its contents and answer the following questions:
    
- What is the "Common Name" (CN) listed in the certificate's Subject?
...
objective = 'Use the OpenSSL command-line tool to inspect the details of a website's digital certificate and answer specific questions about its contents.'
evidence = 'Submit your answers to all the questions.'
...
```text
I answered the required sub-tasks for "The Examiner" and submitted the requested format.
```
