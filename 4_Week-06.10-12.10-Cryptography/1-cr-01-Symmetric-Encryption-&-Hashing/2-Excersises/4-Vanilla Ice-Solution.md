...
**Goal:**  
Decrypt a file that has been encrypted using repeating-key XOR by discovering the key.

**Requirements:**

- **Calculate Hamming Distance:** First, write a function to compute the Hamming distance between two strings. The Hamming distance is the number of differing bits. For example, the distance between `this is a test` and `wokka wokka!!!` is `37`. Verify your function with this example.
    
- **Find the Key Size:** To find the most likely key size, you will:
    
- Iterate through possible key sizes, for example, from 2 to 40.
    
- For each key size, take the first block of that size and the second block of that size from the ciphertext.
...
objective = 'Decrypt a file that has been encrypted using repeating-key XOR by discovering the key.'
evidence = 'The repeating key you discovered.'
...
```text
I answered the required sub-tasks for "Vanilla Ice" and submitted the requested format.
```
