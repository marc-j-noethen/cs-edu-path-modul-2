...
**Goal:**  
Use `openssl` to create a digital signature for a file to ensure its integrity, verify it successfully, and then demonstrate what happens when the file is altered.

**Requirements:**

- For this task, generate a completely new key pair: a private key named `signer_private.pem` and its corresponding public key `signer_public.pem`.
    
- Create a file named `manifest.txt`. Inside it, write the text "Package contents: 1x rubber chicken, 1x vial of invisible ink."
    
- Create a digital signature for `manifest.txt` using your new private key. Name the signature file `manifest.sig`.
    
- Verify that the signature is valid for the original, unaltered file.
...
objective = 'Use `openssl` to create a digital signature for a file to ensure its integrity, verify it successfully, and then demonstrate what happens when the file is altered.'
evidence = 'Submit two screenshots:'
...
```text
I answered the required sub-tasks for "The Unbroken Seal" and submitted the requested format.
```
