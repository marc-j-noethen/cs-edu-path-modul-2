...
**Goal:**  
Practice the fundamental asymmetric encryption/decryption workflow using the `openssl` command-line tool. You will create a key pair, encrypt a secret message, and finally decrypt it.

**Requirements:**

- Create your own public/private key pair. Generate a 2048-bit RSA private key named `my_private_key.pem`.
    
- From your private key, extract the corresponding public key and save it as `my_public_key.pem`.
    
- Now, create a message. Make a new file named `secret.txt` containing the text "Asymmetric encryption is the foundation of modern trust on the web."
    
- Encrypt `secret.txt` using your public key. Name the output file `secret.enc`.
...
objective = 'Practice the fundamental asymmetric encryption/decryption workflow using the `openssl` command-line tool. You will create a key pair, encrypt a secret message, and finally decrypt it.'
evidence = 'Submit a single screenshot showing the command you used to decrypt the file and the resulting original message printed to the console.'
...
```text
I answered the required sub-tasks for "For Your Eyes Only" and submitted the requested format.
```
