...
**Goal:**  
Successfully intercept and decrypt HTTPS traffic from the Windows VM. When you attempt to visit an HTTPS site through your proxy, the browser will show a certificate trust error because it doesn't trust your proxy's dynamically generated certificates.

Your task is to solve this trust issue. You must make the Windows VM's browser trust the certificate authority (CA) used by your proxy tool.

Your final objective is to capture the plaintext credentials submitted to a secure login form, for example `https://github.com/login`.
...
objective = 'Successfully intercept and decrypt HTTPS traffic from the Windows VM. When you attempt to visit an HTTPS site through your proxy, the browser will show a certificate trust error because it doesn't trust your proxy's dynamically generated certificates.

Your task is to solve this trust issue. You must make the Windows VM's browser trust the certificate authority (CA) used by your proxy tool.

Your final objective is to capture the plaintext credentials submitted to a secure login form, for example `https://github.com/login`.'
evidence = 'Submit a single screenshot of the tool on your Kali machine showing the decrypted content of the HTTPS POST request, clearly displaying the captured username and password.'
...
```text
I answered the required sub-tasks for "A Matter of Trust" and submitted the requested format.
```
